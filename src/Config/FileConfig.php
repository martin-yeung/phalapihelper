<?php
namespace MartinYeung\PhalapiHelper\Config;
...
class FileConfig implements Config {
	...
	private function loadConfig($fileName) {
        $fileNameFinal = $this->path . DIRECTORY_SEPARATOR . $fileName . '.php';

        if(defined('APP_ENV')) {
            $fileNameTemp = $fileName .'_'. APP_ENV;
            $fileNameTemp = $this->path . DIRECTORY_SEPARATOR . $fileNameTemp . '.php';
            if(file_exists($fileNameTemp)) {
                $fileNameFinal = $fileNameTemp;
            }
        }

		$config = include($fileNameFinal);
